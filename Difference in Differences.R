...
# Cited by: https://rpubs.com/phle/r_tutorial_difference_in_differences
...
# In April 1992, the U.S. state of New Jersey (NJ) raised the minimum wage from $4.25 to $5.05. 
# Card and Krueger (1994) use a DiD approach and show that this increase in minimum wages led to an increase in employment in the sector of fast food restaurants.
...
# Add variable names
data_mod <- data_raw %>%
  dplyr::select(-X47) %>% # remove empty column
...
data_mod <- data_mod %>%
  ...
  dplyr::select(-southj, -centralj, -northj, -shore, -pa1, -pa2) %>% # now included in region dummy
  mutate(date2 = lubridate::mdy(date2)) %>% # Convert date
  dplyr::rename(open2 = open2r) %>% #Fit name to wave 1
  dplyr::rename(firstinc2 = firstin2) %>% # Fit name to wave 1
...
structure <- data_mod %>%
  dplyr::select(sheet, chain, co_owned, state, region)

# Wave 1 variables
wave1 <- data_mod %>%
  dplyr::select(-ends_with("2"), - names(structure)) %>%
...
card_krueger_1994 <- bind_rows(wave1, wave2) %>%
  dplyr::select(sort(names(.))) %>% # Sort columns alphabetically
...
# Pre-treatment means
card_krueger_1994_mod %>%
  dplyr::filter(observation == "February 1992") %>%
  dplyr::group_by(state) %>%
  dplyr::summarise(emptot = mean(emptot, na.rm = TRUE),
...
hist.feb <- card_krueger_1994_mod %>%
  ...
  geom_histogram(aes(y=c(..count..[..group..==1]/sum(..count..[..group..==1]), # Calculation of group percentage
...
ggarrange(hist.feb, hist.nov, ncol = 2, # ggarrange from ggpubr allows to combine two plots into a single plot with a single legend
...
# Average treatment effect on treated ATT in two ways
